...
expdir=experiments_new/og-nec-intel-otm4-noftrace-advisor
if [ ! -d $expdir ]; then
 mkdir $expdir
fi
...
for ap in bt cg ep ft is mg sp ua; do
   ...
   if [[ -e $pgmx ]]; then
	...
	res1=$(date +"%s.%3N")
      if [[ $cf == *"intel"* ]]; then
         echo "running intel tracing"

         perf stat -o $outdir/perf.${ap}.out.txt -e L1-dcache-loads,L1-dcache-load-misses,L1-icache-load-misses,LLC-loads,LLC-load-misses -M GFLOPs $pgmx !&>> $tmpf
         advisor --collect=roofline --project-dir=$outdir/ -- $pgmx !&>> $tmpf
         advisor --report=roofline --project-dir=$outdir/ --format=text --report-output=$outdir/advisor.${ap}.out.txt 
      else
         echo "running nec"
         $pgmx !&>> $tmpf
      fi
